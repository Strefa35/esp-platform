...
static const char* TAG = "ESP::ETH";
  ...
  #define SPI_ETHERNETS_NUM           CONFIG_ETH_CTRL_SPI_ETHERNETS_NUM
#else
...
#define INIT_SPI_ETH_MODULE_CONFIG(eth_module_config, num)                              \
  do {                                                                                  \
    eth_module_config[num].spi_cs_gpio = CONFIG_ETH_CTRL_SPI_CS ##num## _GPIO;          \
    eth_module_config[num].int_gpio = CONFIG_ETH_CTRL_SPI_INT ##num## _GPIO;            \
    eth_module_config[num].polling_ms = CONFIG_ETH_CTRL_SPI_POLLING ##num## _MS;        \
    eth_module_config[num].phy_reset_gpio = CONFIG_ETH_CTRL_SPI_PHY_RST ##num## _GPIO;  \
    eth_module_config[num].phy_addr = CONFIG_ETH_CTRL_SPI_PHY_ADDR ##num;               \
  } while(0)
...
static void ethctrl_IpEventHandler(void *arg, esp_event_base_t event_base,
                                   int32_t event_id, void *event_data)
{
...
static esp_err_t spi_bus_init(void)
{
    ...
    spi_bus_config_t buscfg = {
        .miso_io_num = CONFIG_ETH_CTRL_SPI_MISO_GPIO,
        .mosi_io_num = CONFIG_ETH_CTRL_SPI_MOSI_GPIO,
        .sclk_io_num = CONFIG_ETH_CTRL_SPI_SCLK_GPIO,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
    };
    ESP_GOTO_ON_ERROR(spi_bus_initialize(CONFIG_ETH_CTRL_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO),
                        err, TAG, "SPI host #%d init failed", CONFIG_ETH_CTRL_SPI_HOST);
 ...
 *          - NULL if init failed
 */
static esp_eth_handle_t ethctrl_InitSpi(spi_eth_module_config_t *spi_eth_module_config, esp_eth_mac_t **mac_out, esp_eth_phy_t **phy_out)
{
    esp_eth_handle_t ret = NULL;
    ...
    spi_device_interface_config_t spi_devcfg = {
        .mode = 0,
        .clock_speed_hz = CONFIG_ETH_CTRL_SPI_CLOCK_MHZ * 1000 * 1000,
        .queue_size = 20,
...
#if CONFIG_ETH_CTRL_USE_KSZ8851SNL
    eth_ksz8851snl_config_t ksz8851snl_config = ETH_KSZ8851SNL_DEFAULT_CONFIG(CONFIG_ETH_CTRL_SPI_HOST, &spi_devcfg);
...
#elif CONFIG_ETH_CTRL_USE_DM9051
    eth_dm9051_config_t dm9051_config = ETH_DM9051_DEFAULT_CONFIG(CONFIG_ETH_CTRL_SPI_HOST, &spi_devcfg);
...
#elif CONFIG_ETH_CTRL_USE_W5500
    eth_w5500_config_t w5500_config = ETH_W5500_DEFAULT_CONFIG(CONFIG_ETH_CTRL_SPI_HOST, &spi_devcfg);
    w5500_config.int_gpio_num = spi_eth_module_config->int_gpio;
    w5500_config.poll_period_ms = spi_eth_module_config->polling_ms;
    esp_eth_mac_t *mac = esp_eth_mac_new_w5500(&w5500_config, &mac_config);
    esp_eth_phy_t *phy = esp_eth_phy_new_w5500(&phy_config);
#endif //CONFIG_ETH_CTRL_USE_W5500
    // Init Ethernet driver to default and install it
    esp_eth_handle_t eth_handle = NULL;
...
static esp_err_t ethctrl_Init(void) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(ETH_EVENT, ESP_EVENT_ANY_ID, &ethctrl_EthEventHandler, NULL));
  ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_ETH_GOT_IP, &ethctrl_IpEventHandler, NULL));
...
esp_err_t ethctrl_Done(void) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_unregister(ETH_EVENT, ESP_EVENT_ANY_ID, &ethctrl_EthEventHandler));
  ESP_ERROR_CHECK(esp_event_handler_unregister(IP_EVENT, ESP_EVENT_ANY_ID, &ethctrl_IpEventHandler));
