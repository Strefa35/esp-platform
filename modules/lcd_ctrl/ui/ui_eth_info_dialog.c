/**
 * @file ui_eth_info_dialog.c
 * @brief Ethernet info modal (LVGL).
 *
...
/** Close button size vs measured "Close" label (not LV_PCT — that is % of parent, not of text). */
#define CLOSE_BTN_SIZE_PCT  150

/** How often to refresh the Ethernet link label while the dialog is open (ms). */
#define ETH_LINK_REFRESH_MS 400

static lv_obj_t* s_overlay = NULL;
static lv_obj_t* s_eth_link_lbl = NULL;
static lv_timer_t* s_eth_link_timer = NULL;

static void eth_dialog_stop_link_refresh(void) {
  if (s_eth_link_timer != NULL) {
    lv_timer_delete(s_eth_link_timer);
    s_eth_link_timer = NULL;
  }
  s_eth_link_lbl = NULL;
}

static void eth_link_label_set(lv_obj_t* lbl, bool up) {
  if (lbl == NULL) {
    return;
  }
...
static void eth_link_refresh_timer_cb(lv_timer_t* t) {
  (void) t;
  if (s_eth_link_lbl == NULL) {
    return;
  }
  eth_link_label_set(s_eth_link_lbl, lcd_GetEthLinkConnected());
}
...
/**
 * @brief Pixel width and height of @p text using the same font as @p style_ref (optional).
 */
...
static void eth_dialog_close_cb(lv_event_t* e) {
  lv_obj_t* overlay = (lv_obj_t*) lv_event_get_user_data(e);
  if (overlay == NULL) {
    return;
  }
  eth_dialog_stop_link_refresh();
...
/**
 * @brief One horizontal row: short muted label, wrapped value (saves height vs stacked labels).
 */
...
void ui_eth_info_dialog_show(const ui_eth_info_dialog_data_t* data) {
  ...
  /* Scroll viewport: tall enough for wrapped IP/MAC lines on small displays. */
  lv_coord_t scroll_h = (vres * 50) / 100;
  if (scroll_h < 88) {
    scroll_h = 88;
  }
  ...
  /* Title + link on one line — saves one block vs stacked "Link / Connected". */
  ...
  lv_label_set_text(title, "Ethernet");
  lv_obj_set_style_text_color(title, lv_color_hex(COLOR_ACCENT), 0);

  lv_obj_t* link_lbl = lv_label_create(head);
  s_eth_link_lbl = link_lbl;
  eth_link_label_set(link_lbl, lcd_GetEthLinkConnected());
  s_eth_link_timer = lv_timer_create(eth_link_refresh_timer_cb, ETH_LINK_REFRESH_MS, NULL);
  ...
  add_compact_row(inner, "IP", data->ip);
  add_compact_row(inner, "Mask", data->netmask);
  add_compact_row(inner, "GW", data->gateway);
  add_compact_row(inner, "MAC", data->mac);

  /* Close: CLOSE_BTN_SIZE_PCT % of measured "Close" text (font matched to link_lbl). */
  ...
  lv_obj_add_event_cb(close_btn, eth_dialog_close_cb, LV_EVENT_CLICKED, overlay);
