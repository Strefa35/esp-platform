/**
 * @file ui_wifi_info_dialog.c
 * @brief Wi-Fi info modal (LVGL).
 *
...
#define WIFI_LINK_REFRESH_MS 400

static lv_obj_t* s_overlay = NULL;
static lv_obj_t* s_wifi_link_lbl = NULL;
static lv_timer_t* s_wifi_link_timer = NULL;

static void wifi_dialog_stop_link_refresh(void) {
  if (s_wifi_link_timer != NULL) {
    lv_timer_delete(s_wifi_link_timer);
    s_wifi_link_timer = NULL;
  }
  s_wifi_link_lbl = NULL;
}

static void wifi_link_label_set(lv_obj_t* lbl, bool up) {
  if (lbl == NULL) {
    return;
  }
...
static void wifi_link_refresh_timer_cb(lv_timer_t* t) {
  (void) t;
  if (s_wifi_link_lbl == NULL) {
    return;
  }
  wifi_link_label_set(s_wifi_link_lbl, lcd_GetWifiLinkConnected());
}
...
static void wifi_dialog_close_cb(lv_event_t* e) {
  lv_obj_t* overlay = (lv_obj_t*) lv_event_get_user_data(e);
  if (overlay == NULL) {
    return;
  }
  wifi_dialog_stop_link_refresh();
...
void ui_wifi_info_dialog_show(const ui_wifi_info_dialog_data_t* data) {
  ...
  s_wifi_link_lbl = link_lbl;
  wifi_link_label_set(link_lbl, lcd_GetWifiLinkConnected());
  s_wifi_link_timer = lv_timer_create(wifi_link_refresh_timer_cb, WIFI_LINK_REFRESH_MS, NULL);
  ...
  lv_obj_add_event_cb(close_btn, wifi_dialog_close_cb, LV_EVENT_CLICKED, overlay);
